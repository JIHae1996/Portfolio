...
export default function SubBg() {
  return (
    <Canvas shadows gl={{ stencil: false, antialias: false }} camera={{ position: [0, 0, 20], fov: 50, near: 17, far: 40 }}>
      ...
      <EffectComposer>
        <SSAO radius={0.4} intensity={50} luminanceInfluence={0.4} color="#7fa2a5" />
...
function InstancedSpheres({ count = 200 }) {
  ...
  return (
    <instancedMesh ref={ref} castShadow receiveShadow args={[null, null, count]}>
      <sphereBufferGeometry args={[1.5, 20, 20]} />
      <meshLambertMaterial color="#7e7e7e" />
    </instancedMesh>
  )
}
